...
void BaseApplication::handle_error_message(const ErrorEvent &ev) {
  ...
  static std::vector<std::string> flow_mod_failed_code = {
      std::string("OFPFMFC_UNKNOWN"),
      std::string("OFPFMFC_TABLE_FULL"),
      std::string("OFPFMFC_BAD_TABLE_ID"),
      std::string("OFPFMFC_OVERLAP"),
      std::string("OFPFMFC_EPERM"),
      std::string("OFPFMFC_BAD_TIMEOUT"),
      std::string("OFPFMFC_BAD_COMMAND"),
      std::string("OFPFMFC_BAD_FLAGS")
  };
  ...
  static std::vector<std::string> port_mod_failed_code  = {
      std::string("OFPPMFC_BAD_PORT"),
      std::string("OFPPMFC_BAD_HW_ADDR"),
      std::string("OFPPMFC_BAD_CONFIG"),
      std::string("OFPPMFC_BAD_ADVERTISE"),
      std::string("OFPPMFC_EPERM")
  };

  static std::vector<std::string> table_mod_failed_code  = {
      std::string("OFPTMFC_BAD_TABLE"),
      std::string("OFPTMFC_BAD_CONFIG"),
      std::string("OFPTMFC_EPERM")
  };

  static std::vector<std::string> queue_op_failed_code = {
      std::string("OFPQOFC_BAD_PORT"),
      std::string("OFPQOFC_BAD_QUEUE"),
      std::string("OFPQOFC_EPERM")
  };

  static std::vector<std::string>  switch_config_failed_code = {
      std::string("OFPSCFC_BAD_FLAGS"),
      std::string("OFPSCFC_BAD_LEN"),
  ...
  static std::vector<std::string> table_features_failed_code = {
      std::string("OFPTFFC_BAD_TABLE"),
      std::string("OFPTFFC_BAD_METADATA"),
      std::string("OFPTFFC_BAD_TYPE"),
      std::string("OFPTFFC_BAD_LEN"),
      std::string("OFPTFFC_BAD_ARGUMENT"),
      std::string("OFPTFFC_EPERM")
  };
  ...
  OAILOG_ERROR(LOG_GTPV1U,
               ...
               error_type[ev.get_error_type()].c_str(), ev.get_error_type(), error_code[ev.get_error_type()][ev.get_error_code()].c_str(), ev.get_error_code());
#if SERVICE303
  char type_str[50];
  char code_str[50];
