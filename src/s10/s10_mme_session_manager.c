    ...
    itti_s10_forward_relocation_response_t *forward_relocation_response_p)
{
   ...
   if(forward_relocation_response_p->cause == REQUEST_ACCEPTED){
     ...
     ulp_req.apiInfo.createLocalTunnelInfo.teidLocal = forward_relocation_response_p->s10_target_mme_teid.teid;
...
s10_mme_forward_access_context_notification(NwGtpv2cStackHandleT *stack_p,
   ...
   */
  memset (&ulp_req, 0, sizeof (NwGtpv2cUlpApiT));
  ulp_req.apiInfo.initialReqInfo.teidLocal = forward_access_context_notif_p->local_teid;
  ...
  NwGtpv2cUlpApiT * pUlpApi)
{
  ...
  resp_p->trxn = (void *)pUlpApi->apiInfo.triggeredRspIndInfo.hUlpTrxnId;
    ...
    itti_s10_context_acknowledge_t *ack_p)
{
  ...
  ulp_ack.apiInfo.triggeredAckInfo.peerIp = ack_p->peer_ip;
  ulp_ack.apiInfo.triggeredAckInfo.peerPort = ack_p->peer_port;

  /*
   * Prepare a context ack to send to target MME.
   */
  rc = nwGtpv2cMsgNew (*stack_p, NW_TRUE, NW_GTP_CONTEXT_ACK, 0, ack_p->trxnId, &(ulp_ack.hMsg));
  DevAssert (NW_OK == rc);
  ...
  NwGtpv2cUlpApiT * pUlpApi)
{
  ...
  case NW_GTP_CONTEXT_RSP:
  {
    ...
    /** Set the transaction for the triggered acknowledgement. */
    ack_p->trxnId = (uint32_t)pUlpApi->apiInfo.rspFailureInfo.hUlpTrxn;
    /** Set the cause. */
