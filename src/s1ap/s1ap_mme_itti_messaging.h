...
static inline void s1ap_mme_itti_mme_app_initial_ue_check_duplicate_message(
  const sctp_assoc_id_t   assoc_id,
  const sctp_stream_id_t  stream_id,
  const uint32_t          enb_id,
  const enb_ue_s1ap_id_t  new_enb_ue_s1ap_id,
  const mme_ue_s1ap_id_t  mme_ue_s1ap_id,
//  const S1ap_InitialUEMessageIEs_t *  s1ap_InitialUEMessageIEs,
//  uint8_t           mmec,
//  uint32_t          s_tmsi,
  const long              rrc_cause,
  const tai_t      const* tai,
  ...
  bstring                 nas)  // unused
{
  ...
  OAILOG_FUNC_IN (LOG_S1AP);
  message_p = itti_alloc_new_message(TASK_S1AP, MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE);

  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).sctp_assoc_id          = assoc_id;
  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).stream_id          = stream_id;
  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).enb_id                 = enb_id;
  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).new_enb_ue_s1ap_id         = new_enb_ue_s1ap_id;
  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).mme_ue_s1ap_id         = mme_ue_s1ap_id;

  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).is_s_tmsi_valid      = true;
//  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).mmec                 = mmec;
//  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).s_tmsi               = s_tmsi;
  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).as_cause             = rrc_cause + 1;
  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai                  = *tai;
  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).cgi                  = *cgi;

  if (opt_s_tmsi) {
    MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).is_s_tmsi_valid      = true;
    MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).opt_s_tmsi           = *opt_s_tmsi;
   } else {
     MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).is_s_tmsi_valid      = false;
   }
   if (opt_csg_id) {
     MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).is_csg_id_valid      = true;
     MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).opt_csg_id           = *opt_csg_id;
   } else {
     MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).is_csg_id_valid      = false;
   }
   if (opt_gummei) {
     MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).is_gummei_valid      = true;
     MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).opt_gummei           = *opt_gummei;
   } else {
     MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).is_gummei_valid      = false;
   }

//  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).nas_msg_length       = nas_msg_length;
//  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).nas_msg              = nas_msg;
  MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).nas                    = nas; //blk2bstr(nas_msg, nas_msg_length);

  MSC_LOG_TX_MESSAGE(
        MSC_S1AP_MME,
        MSC_MMEAPP_MME,
        NULL,0,
        "0 MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE ue_id "MME_UE_S1AP_ID_FMT" as cause %u tai:%c%c%c.%c%c%c:%u ",
        mme_ue_s1ap_id,
        MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).as_cause,
        (char)(MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai.plmn.mcc_digit1 + 0x30),
        (char)(MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai.plmn.mcc_digit2 + 0x30),
        (char)(MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai.plmn.mcc_digit3 + 0x30),
        (char)(MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai.plmn.mnc_digit1 + 0x30),
        (char)(MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai.plmn.mnc_digit2 + 0x30),
        (9 < MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai.plmn.mnc_digit3) ? ' ': (char)(MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai.plmn.mnc_digit3 + 0x30),
            MME_APP_INITIAL_UE_MESSAGE_CHECK_DUPLICATE(message_p).tai.tac);
  itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
  OAILOG_FUNC_OUT (LOG_S1AP);
}
