  ...
  eutran_vector_t * vector)
{
  ...
  if (ret) {
    OAILOG_ERROR (LOG_S6A, "Missing AVP for E-UTRAN vector: %c%c%c%c\n", ret & 0x01 ? 'R' : '-', ret & 0x02 ? 'X' : '-', ret & 0x04 ? 'A' : '-', ret & 0x08 ? 'K' : '-');
    return RETURNerror;
  }
...
static inline int
s6a_parse_authentication_info_avp (
...
int
s6a_generate_authentication_info_req (
