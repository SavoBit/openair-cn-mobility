...
static inline int
s6a_parse_network_access_mode (
  struct avp_hdr *hdr_network_am,
...
static inline int
s6a_parse_access_restriction_data (
...
static inline int
s6a_parse_all_apn_conf_inc_ind (
  struct avp_hdr *hdr,
...
static inline int
s6a_parse_service_selection (
  struct avp_hdr *hdr_service_selection,
...
static inline int
s6a_parse_allocation_retention_priority (
  struct avp *avp_arp,
...
static inline int
s6a_parse_eps_subscribed_qos_profile (
  struct avp *avp_qos,
...
static inline int
s6a_parse_apn_configuration (
  struct avp *avp_apn_conf_prof,
  apn_configuration_t * apn_config)
{
  ...
  while (avp) {
    ...
    case AVP_CODE_SERVICE_SELECTION:
      CHECK_FCT (s6a_parse_service_selection (hdr, apn_config->service_selection, &apn_config->service_selection_length));
      break;
...
static inline int
s6a_parse_apn_configuration_profile (
  struct avp *avp_apn_conf_prof,
  apn_config_profile_t * apn_config_profile)
{
  ...
  while (avp) {
    ...
    case AVP_CODE_APN_CONFIGURATION:{
        DevCheck (apn_config_profile->nb_apns < MAX_APN_PER_UE, apn_config_profile->nb_apns, MAX_APN_PER_UE, 0);
        CHECK_FCT (s6a_parse_apn_configuration (avp, &apn_config_profile->apn_configuration[apn_config_profile->nb_apns]));
  ...
  subscription_data_t * subscription_data)
{
  ...
  while (avp) {
    ...
    case AVP_CODE_MSISDN:
      CHECK_FCT (s6a_parse_msisdn (hdr, subscription_data->msisdn, &subscription_data->msisdn_length));
      break;
