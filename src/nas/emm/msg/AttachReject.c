  ...
  uint32_t len)
{
  ...
  while (len - decoded > 0) {
    ...
    case ATTACH_REJECT_ESM_MESSAGE_CONTAINER_IEI:
      if ((decoded_result = decode_esm_message_container (&attach_reject->esmmessagecontainer, ATTACH_REJECT_ESM_MESSAGE_CONTAINER_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
  ...
  uint32_t len)
{
  ...
  if ((encode_result = encode_emm_cause (&attach_reject->emmcause, 0, buffer + encoded, len - encoded)) < 0)    //Return in case of error
