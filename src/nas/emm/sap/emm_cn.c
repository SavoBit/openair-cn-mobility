...
extern int emm_cn_wrapper_ho_forward_relocation_request(emm_cn_ho_forward_relocation_req_t * msg_pP);
extern int emm_cn_nas_ho_forward_relocation_request_failed(mme_ue_s1ap_id_t        ue_id);
...
static int _emm_cn_ho_bearer_modification_res (emm_cn_ho_bearer_mod_res_t * msg_pP)
{
  int                                     rc = RETURNerror;
  struct emm_data_context_s              *emm_ctx_p = NULL;
  esm_proc_pdn_type_t                     esm_pdn_type = ESM_PDN_TYPE_IPV4;
...
//  data_p->esm_msg = rsp;
   ...
   * Send attach accept message to the UE
   */
  // todo: this is the X2AP HO case (single MME handover)..
//  rc = emm_cn_wrapper_mobility(emm_ctx_p, data_p);
//
//  if (rc != RETURNerror) {
//    if (IS_EMM_CTXT_PRESENT_OLD_GUTI(emm_ctx_p) &&
//        (memcmp(&emm_ctx_p->_old_guti, &emm_ctx_p->_guti, sizeof(emm_ctx_p->_guti)))) {
...
static int _emm_cn_ho_bearer_modification_fail (emm_cn_ho_bearer_mod_fail_t * msg_pP)
{
  int                                     rc = RETURNerror;
  struct emm_data_context_s              *emm_ctx_p = NULL;
  esm_proc_pdn_type_t                     esm_pdn_type = ESM_PDN_TYPE_IPV4;
...
// todo: merging this with emm_proc_attach_request?
static int _emm_cn_ho_forward_relocation_req (emm_cn_ho_forward_relocation_req_t * msg_pP)
{
  int                                     rc = RETURNerror;
  ...
  /** Clear the stack EMM_CTX. */
  memset (&ue_ctx, 0, sizeof (emm_data_context_t));
  ue_ctx.is_dynamic = false;
  ue_ctx.ue_id      = msg_pP->ue_id;

  /**
   * Check that no EMM Data context exists by IMSI or MME_UE_ID.
   * todo: could there be cases where a context may exist?
   */
  emm_ctx_p = emm_data_context_get (&_emm_data, msg_pP->ue_id);
  if (emm_ctx_p != NULL) {
    OAILOG_ERROR (LOG_NAS_EMM, "EMMCN-SAP  - " "UE context with MME_UE_S1AP_ID: " MME_UE_S1AP_ID_FMT " already present. Cannot create new one based on S10 Forward Relocation Request...\n", msg_pP->ue_id);
  ...
  emm_ctx_p = emm_data_context_get_by_imsi(&_emm_data, msg_pP->imsi);
  if (emm_ctx_p != NULL) {
      OAILOG_ERROR (LOG_NAS_EMM, "EMMCN-SAP  - " "UE context with IMSI: " IMSI_64_FMT " already present. Cannot create new one based on S10 Forward Relocation Request...\n", msg_pP->ue_id);
      /** Inform the MME_APP layer of the failed relocation in the NAS layer. */
      emm_cn_nas_ho_forward_relocation_request_failed(msg_pP->ue_id);
      /** Remove the current NAS/EMM UE context. */
      _clear_emm_ctxt(emm_ctx_p);
  ...
  /** Handle the rest in the Handover.c file. */
  rc = emm_cn_wrapper_ho_forward_relocation_request(msg_pP);
  /** Successfully registered UE context. */
  OAILOG_FUNC_RETURN (LOG_NAS_EMM, rc);
}
...
int emm_cn_send (const emm_cn_t * msg)
{
  ...
  // handover messaging: handle handover triggered CSR or MBR in the same bearer modification message..
  case EMMCN_HO_BEARER_MODIFICATION_RES:
    rc = _emm_cn_ho_bearer_modification_res (msg->u.emm_cn_ho_bearer_mod_res);
    break;

  case EMMCN_HO_BEARER_MODIFICATION_FAIL:
    rc = _emm_cn_ho_bearer_modification_fail (msg->u.emm_cn_ho_bearer_mod_fail);
    break;

  /** S1AP Handover Forward Relocation Request Messages. */
  case EMMCN_HO_FORWARD_RELOCATION_REQ:
    rc = _emm_cn_ho_forward_relocation_req(msg->u.emm_cn_ho_forward_relocation_req);
    break;
